...
public class RssService {
    ...
    public int fetchAndProcessAllFeeds() {
        List<ArticleEntity> allNewsItems = new ArrayList<>();

        RssSource.getEnabledSources().forEach(source -> {
            try {
                List<ArticleEntity> items = fetchFeedFromSource(source);
     ...
     * @return 변환된 ArticleEntity 엔티티 목록
     * @throws Exception RSS 피드 로드 및 처리 중 발생할 수 있는 예외
     */
    private List<ArticleEntity> fetchFeedFromSource(RssSource source) {
        List<ArticleEntity> newsItems = new ArrayList<>();
        try {
            ...
            for (SyndEntry entry : feed.getEntries()) {
                try {
                    ArticleEntity newsItem = mapper.mapToRssNews(entry, source);
                    newsItems.add(newsItem);
                } catch (Exception e) {
                    log.warn("[{}] 항목 변환 실패: {} - {}",
                            ArticleCollectorErrorCode.ITEM_MAPPING_ERROR.getCode(),
            ...
            log.error("[{}] 피드 로드 실패: {} - {}",
                    ArticleCollectorErrorCode.FEED_CONNECTION_ERROR.getCode(),
     ...
     */
    @Transactional
    public int saveItems(List<ArticleEntity> newsItems) {
        int savedCount = 0;

        List<ArticleEntity> newItems = newsItems.stream()
                .filter(not(item -> rssNewsRepository.existsByLink(item.getLink())))
                .collect(Collectors.toList());

        for (ArticleEntity item : newItems) {
            try {
