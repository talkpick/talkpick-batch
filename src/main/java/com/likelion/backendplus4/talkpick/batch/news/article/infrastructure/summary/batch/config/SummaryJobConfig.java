...
public class SummaryJobConfig {
	...
	private final Step summaryTotalPageCheckStep;
	private final Step articleSummaryStep;

	public SummaryJobConfig(JobRepository jobRepository, Step summaryTotalPageCheckStep, Step articleSummaryStep) {
		this.jobRepository = jobRepository;
		this.summaryTotalPageCheckStep = summaryTotalPageCheckStep;
	...
	public Job summaryJob() {
		return new JobBuilder(jobName, jobRepository)
			.start(summaryTotalPageCheckStep)
				.on("ITEMS_NOT_FOUND").end()
			.from(summaryTotalPageCheckStep)
			.on("*").to(articleSummaryStep)
			.end()
			.build();
	}
}
